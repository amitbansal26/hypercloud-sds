...
declare -A PL_VMs
PL_VMs=( ["ProLinux_node1"]="172.22.4.201" \
         ["ProLinux_node2"]="172.22.4.202" \
         ["ProLinux_node3"]="172.22.4.203" )
VM="VirtualBox"
DEPLOYED_KUBE=( "prolinux_v1.15.3_calico" \
                "prolinux_v1.16.8_calico" \
                "prolinux_v1.15.3_flannel" \
                "prolinux_v1.16.8_flannel" )
...
deploying_kube(){
    ...
    ansible-playbook -i inventory/mycluster/hosts.yaml --become --become-user=root cluster.yml -e \
        "ansible_user=root kube_version_min_required=v1.15.0 kube_version=$1 kube_network_plugin=$2"
    popd
    set +eo pipefail
